...
cells match within roughly 0.03. Extending the sample, we find the problem the
...
enters CRSP in 1962 and NASDAQ at the end of 1972. As a robustness check we rebuilt 
a NYSE-only value-weighted index directly from the CRSP monthly stock file, 
weighting each security's return by its lagged market capitalization.
Over the full overlapping sample the two dividend-price
series correlate at 0.994, and the predictive slopes differ by 0.014
(0.129 for the total-market index against 0.143 for the NYSE-only index);
Table~\ref{tab:nyse-index-comparison} reports the comparison. The index choice
does not materially affect the results.

Table~\ref{tab:summary-stats} and Figure~\ref{fig:summary-stats-dp} describe the
resulting panel.

\input{../_output/summary_stats.tex}

\input{../_output/summary_stats_dp_figure.tex}

\paragraph{A note on numbering.} Our exhibit numbers necessarily differ from
the paper's, since we add exhibits of our own. Throughout, our
Table~\ref{tab:table1} corresponds to the paper's Table 1, our
Table~\ref{tab:table2} to the paper's Table 2, and our
Figure~\ref{fig:figure1} to the paper's Figure 1.

\section{Replication results}

Table~\ref{tab:table1} reports the finite-sample properties of the OLS
predictive slope for each of the paper's four subsamples, in the paper's own
...
Figure~\ref{fig:figure1} reproduces the geometry of Stambaugh's original
Figure~1. Our bias-adjusted points correspond to the paper's ``F'' points 
and match closely:
...
\input{../_output/update_comparison.tex}

Tables~\ref{tab:table1_updated} and~\ref{tab:table2_updated} repeat every
exhibit on the extended samples. Three results stand out. First, the bias behaves as theory predicts: in the full
...
Table~\ref{tab:update-comparison} condenses the comparison into a single view, 
reporting the OLS and bias-adjusted slopes, both persistence estimates, 
and both $p$-values side by side for the paper era, the full updated sample, and the post-paper period.
...
the estimated slope. The Bayesian posterior for the same window places 1\% or 
less of its mass below zero depending on the specification, so the divergence 
between frameworks has widened as well. We also note that the stationarity restriction has become nearly
...
specifications, and the geometry of Figure~\ref{fig:figure1} reproduce within a few hundredths,
including the paper's most extreme individual result.

\input{../_output/nyse_index_comparison.tex}
...
to 0.989, displacing that specification's position in Figure~\ref{fig:figure1}. 
The diagnosis is that the conditional posterior,
